...
let keyPromise = null;
export async function encryptDocumentAtRest(record) {
    ...
    const ciphertext = sodium.crypto_aead_xchacha20poly1305_ietf_encrypt(JSON.stringify(envelope), null, null, nonce, key);
    return {
...
export async function decryptDocumentAtRest(record) {
    ...
    const ciphertext = sodium.from_base64(record.encryptedContent, sodium.base64_variants.ORIGINAL);
    const envelopeJson = sodium.crypto_aead_xchacha20poly1305_ietf_decrypt(null, ciphertext, null, nonce, key);
    const envelope = JSON.parse(sodium.to_string(envelopeJson));
    if (envelope.version !== 1) {
        throw new Error("Unsupported server encryption envelope");
    }
    const decryptedRecord = JSON.parse(envelope.payload);
...
async function getServerEncryptionKey() {
    if (!keyPromise) {
        keyPromise = (async () => {
            await sodium.ready;
            ...
            if (key.length !== expectedLength) {
                throw new Error(`SERVER_ENCRYPTION_KEY must decode to ${expectedLength} bytes for XChaCha20-Poly1305.`);
            }
            return key;
        })();
    }
